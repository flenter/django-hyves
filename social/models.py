...
class UserAssociationManager(models.Manager):
    """Manager for the UserAssociation model. Makes it easier to create/update
    an object through an oauth access token
    """
    def get_or_create_from_token(self, token):
        ...
        if(token.expiredate):
            try:
                db_token.expires = datetime.datetime.fromtimestamp(
                        token.expiredate
                    )
            except ValueError:
                db_token.expires = \
